...
class MyEWC(BaseLearner):
    def __init__(self, args):
        ...
        self.r_drop = nn.Dropout(p=0.4)#0.6#0.4#0.6#0.4#0.5#0.5
        self.f_drop = nn.Dropout(p=0.2)#0.4#0.2#0.2#0.0#0.2#0.3
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if 20 >= epoch >= 0:
                    #comfeat[:,:1024] = torch.zeros_like(comfeat[:,:1024]).normal_(0, comfeat[:,:1024].std().item() + 1e-12)
                      """for name, parms in self._network.named_parameters():
                        if '.rgb.' in name or '.flow.' in name:
                            parms.requires_grad = False"""
                ...
                #comfeat[:,:1024] = comfeat[:,:1024] + torch.zeros_like(comfeat[:,:1024]).normal_(0, 0.1*comfeat[:,:1024].mean().item() + 1e-12)
                ...
                """if epoch > 15:
                      ...
                      th_u_f = comfeat[:,2048:3072].mean() + k1* comfeat[:,2048:3072].std()
                      th_d_f = comfeat[:,2048:3072].mean() - k1 * comfeat[:,2048:3072].std()

                      comfeat[:,2048:3072][th_d_f > comfeat[:,2048:3072]]= 0
                      comfeat[:,2048:3072][comfeat[:,2048:3072]> th_u_f] = 0
                      comfeat[:,2048:3072] = comfeat[:,2048:3072]
                      for name, parms in self._network.named_parameters():
                        if '.stft.' in name:
                            parms.requires_grad = False"""
                ...
                tot_norm = r_max_grad + f_max_grad
