...
class MyEWC(BaseLearner):
    def __init__(self, args):
        ...
        self.transform = self.transform = nn.Sequential(
            RandomResizedCrop(size=(224, 224), scale=(0.9, 1.)),
            ColorJitter(),
    ...
    def _init_train(self, train_loader, test_loader, optimizer, scheduler,args):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                time_batch = comfeat.size()[0]
               
                f1, f2 = torch.split(comfeat, [int(time_batch/2), int(time_batch/2)], dim=0)
                ...
                logits = self._network.fc(f1)["logits"]"""
                ...
                """for name, parms in self._network.named_parameters():
                        if '.rgb.' in name and parms.grad is not None:
                            parms.grad = parms.grad * 1 + torch.zeros_like(parms.grad).normal_(0, parms.grad.std().item() + 1e-12)
                        elif '.flow.' in name and parms.grad is not None:
                            parms.grad = parms.grad * 5 + torch.zeros_like(parms.grad).normal_(0, parms.grad.std().item() + 1e-12)   
                        elif '.stft.' in name and parms.grad is not None:
                            parms.grad = parms.grad * 100  + torch.zeros_like(parms.grad).normal_(0, parms.grad.std().item() + 1e-12)          
...
##################################################################################
                ...
                for i in range(proto_features.shape[0]):
                    for j in range(8):
                        #for dim in range(1024*len(self._modality)):
...
##############################################################plastic
    ...
    def _compute_accuracy(self, model, loader):
        ...
        digits_final = TSNE(perplexity=50,early_exaggeration=15,n_iter=5000,learning_rate=500,method='exact',init='pca').fit_transform(feature_tsne)
    ...
    def init_eval(self):
        ...
        return loss_rgb*100/140, loss_flow*100/140, loss_acc*100/18, loss_gyro*100/18
    ...
    def protoSave(self, model, train_loader, data_manager):
        ...
        with torch.no_grad():
            for class_idx in range(self._known_classes, self._total_classes):
              ...
              if len(self._modality) > 1:   
                ...
                """rgb_vec = vectors[:,:1024]
                flow_vec = vectors[:,1024:2048]
                acc_vec = vectors[:,2048:3072]
                gyro_vec = vectors[:,3072:4096]
                rgb_vec_class_mean = np.mean(rgb_vec,axis=0)
                flow_vec_class_mean = np.mean(flow_vec,axis=0)
                acce_vec_class_mean = np.mean(acc_vec,axis=0)
                gyro_vec_class_mean = np.mean(gyro_vec,axis=0)"""
            ...
            if len(self._modality) > 1:    
              ...
              """self.rgb_radiuses.append(np.trace(rgb_cov)/rgb_vec.shape[1]) 
              self.flow_radiuses.append(np.trace(flow_cov)/flow_vec.shape[1]) 
              self.acc_radiuses.append(np.trace(acc_cov)/acc_vec.shape[1]) 
              self.gyro_radiuses.append(np.trace(gyro_cov)/rgb_vec.shape[1]) """
              #self._radius = np.sqrt(np.mean(self._radiuses))
              """self.rgb_radius = np.sqrt(np.mean(self.rgb_radiuses))
              self.flow_radius = np.sqrt(np.mean(self.flow_radiuses))
              self.acc_radius = np.sqrt(np.mean(self.acc_radiuses))
              self.gyro_radius = np.sqrt(np.mean(self.gyro_radiuses))"""
            #else:
              #self._radiuses.append(np.trace(cov)/vectors.shape[1]) 
              #self._radius = np.sqrt(np.mean(self._radiuses))
            """
        ...
        labels_set = np.zeros([4]).astype(int)
        for cls_lab in range(len(labels)):
            labels_set[cls_lab] = labels[cls_lab][1]
        labels = np.array(labels)
        labels = np.concatenate([labels[0],labels[1],labels[2],labels[3]],axis=0)
        #labels = np.reshape(labels, labels.shape[0] * labels.shape[1])
        features = np.array(features)
        features = np.concatenate([features[0],features[1],features[2],features[3]], axis = 0)
...
def tsne_plot(x, colors ,task_num):
    ...
    palette = np.array(sns.color_palette("Paired", task_num))
    ...
    for i in range(task_num):
        ...
        txts.append(txt)
    plt.savefig('./tsne_rgb' + str(task_num) + '.png', dpi=400)
    return f, ax, txts