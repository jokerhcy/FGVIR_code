...
class DummyDataset(Dataset):
    ...
    def _load_data(self, modality, record, idx):
        if modality == 'RGB' or modality == 'RGBDiff':
            ...
            flow_path = record.path.replace('/home/amax/Downloads/whx/temporal-binding-network/dataset/data/', '/home/amax/Downloads/whx/temporal-binding-network/dataset/flow/')
            try:
                #hcypath = os.path.join(flow_path, self.image_tmpl[modality].format('x', idx))
                x_img = Image.open(os.path.join(flow_path, self.image_tmpl[modality].format('flow_x', idx))).convert('L')
                y_img = Image.open(os.path.join(flow_path, self.image_tmpl[modality].format('flow_y', idx))).convert('L')
                return [x_img, y_img]
            except Exception:
        ...
        elif modality == 'STFT' or modality == 'STFT_2':
            #stft_path = record.path.replace('/home/amax/Downloads/whx/temporal-binding-network/dataset/data/','/home/amax/Downloads/whx/temporal-binding-network/dataset/stft_param/' )
            stft_path = record.path.replace('/home/amax/Downloads/whx/temporal-binding-network/dataset/data/','/home/amax/Downloads/whx/temporal-binding-network/dataset/stft_param_w16_x512/')
            try:
                """
                img1 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 1))).convert('L')
                img2 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 2))).convert('L')
                img3 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 3))).convert('L')
                img4 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 4))).convert('L')
                img5 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 5))).convert('L')
                img6 = Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 6))).convert('L')
                img1_1 = transforms.ToTensor()(img1)
                img2_1 = transforms.ToTensor()(img2)
                img3_1 = transforms.ToTensor()(img3)
                img4_1 = transforms.ToTensor()(img4)
                img5_1 = transforms.ToTensor()(img5)
                img6_1 = transforms.ToTensor()(img6)
                """

                #ooo = os.path.join(stft_path, self.image_tmpl[modality].format('sensor',idx))
                stft_PIL = [Image.open(os.path.join(stft_path, self.image_tmpl[modality].format('sensor',idx))).convert('L')]
                return stft_PIL
                #stftimg = torch.stack([img1_1,img2_1,img3_1,img4_1,img5_1,img6_1], dim=0).squeeze(1)
                #SSS = stftimg.convert('L')
                #stft_pIL = transforms.ToPILImage(stftimg)
                #return stft_pIL
            except Exception:
                print('error loading flow image:', os.path.join(stft_path, self.image_tmpl[modality].format('sensor', 1)))
    ...
    def get(self, modality, record, indices):
        ...
        if modality != 'Gyro' and modality != 'Acce' and modality != 'STFT' and modality != 'STFT_2':
            images = list()
            ...
            for k in range(3):
                ...
                for i in range(self.new_length[modality]):
                    seg_imgs = self._load_data(modality, record, p)
                    images.extend(seg_imgs) # type: ignore

            process_data = self.transform[modality](images)
        
        elif modality == 'STFT_2':
            images = list()
            for k in range(3):
                p = k+4
                for i in range(self.new_length[modality]):
