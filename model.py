...
class TBN(nn.Module):
    ...
    def __init__(self, num_segments, modality,
        ...
        #is_stft = any(m == 'STFT' for m in self.modality)
        if is_flow:
        ...
        #if  is_stft:
        #    print("Init STFT model weight")
       #     self.base_model['STFT'].apply(self.weights_init)
        #    print("Done. STFT model ready.")
        print('\n')
    ...
    def _prepare_base_model(self, base_model):
        ...
        if base_model == 'BNInception':
            ...
            for m in self.modality:
                if m != 'Gyro' and m != 'Acce' and m != 'STFT' and m!='STFT_2':
                    ...
                    self.base_model[m] = getattr(tf_model_zoo, 'BNInception_STFT')()
                    self.base_model[m].last_layer_name = 'fc'
                    self.input_size[m] = 64
                    self.input_std[m] = [65.0,69.6,72.5]                 
                elif m == 'STFT_2':
                    self.base_model[m] = getattr(tf_model_zoo, 'BNInception_STFT')()
                    self.base_model[m].last_layer_name = 'fc'
                    self.input_size[m] = 64
                    self.input_std[m] = [53,52,53]
                ...
                if m == 'Flow':
                    ...
                    self.input_mean[m] = [53.8545,53.3971,53.8203]       
                elif m == 'STFT_2':
                    self.input_mean[m] = [57.3787,56.8113,57.2249]
        ...
        if 'RGBDiff' in self.modality:
            augmentation['RGBDiff'] = torchvision.transforms.Compose(
                [GroupMultiScaleCrop(self.input_size['RGBDiff'], [1, .875, .75]),
                 GroupRandomHorizontalFlip(is_flow=False)])
        if 'STFT' in self.modality:
            augmentation['STFT'] = torchvision.transforms.Compose(
                [GroupMultiScaleCrop(self.input_size['STFT'], [1, .875, .75]),
                 GroupRandomHorizontalFlip(is_flow=False)
                 ])
        if 'STFT_2' in self.modality:
            augmentation['STFT_2'] = torchvision.transforms.Compose(
                [GroupMultiScaleCrop(self.input_size['STFT_2'], [1, .875, .75]),
                 GroupRandomHorizontalFlip(is_flow=False)
                 ])
        return augmentation
